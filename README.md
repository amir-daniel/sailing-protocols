# Sailing Protocols — Sailing Checklists & Protocols

A structured, iteratively-built library of sailing checklists and protocols,
inspired by the philosophy in Atul Gawande's *The Checklist Manifesto*.

## Checklist philosophy

Two checklist styles are used throughout this repo:

- **DO–CONFIRM** — Do the tasks from memory/experience, then pause and use the
  list to *confirm* nothing was missed. Best for experienced crew and routine ops.
- **READ–DO** — Read each step and perform it as you go. Best for rare, complex,
  or high-stress procedures (e.g. emergencies).

Good checklists are **short**, focus on the **killer items** (the steps that are
easy to miss and dangerous to skip), and **support** judgment rather than replace it.

Each file marks its intended style near the top.

## Categories

| Category | Purpose |
| --- | --- |
| [Boat profile](00-boat-profile/boat-profile.md) | Your specific boat's details — customise the generic checklists |
| [Pre-departure](01-pre-departure/passage-planning.md) | Planning, boat & safety checks, crew brief |
| [Sail handling](02-sail-handling/hoisting-mainsail.md) | Hoisting, reefing, dropping, headsail changes |
| [Maneuvers](03-maneuvers/tacking.md) | Tacking, gybing, heaving-to, anchoring, mooring |
| [Docking & berthing](04-docking-berthing/departure-from-berth.md) | Arrival, departure, spring lines |
| [Emergencies](05-emergencies/man-overboard.md) | MOB, fire, flooding, dismasting, abandon ship, medical |
| [Heavy weather](06-heavy-weather/heavy-weather-preparation.md) | Storm preparation and tactics |
| [Navigation & watch](07-navigation-watch/watch-handover.md) | Passage planning, watch handover, night sailing |
| [Systems & engine](08-systems-engine/engine-pre-start-start.md) | Engine, electrical, through-hulls |
| [Maintenance](09-maintenance/daily-checks.md) | Periodic and seasonal checks |
| [Radio protocols](10-radio-protocols/mayday-distress.md) | Mayday, PAN-PAN, Sécurité, DSC, radio checks |
| [COLREGs quick-reference](11-colregs-quick-reference/right-of-way-steering-rules.md) | Lights, shapes, sound signals, right of way |
| [Supplementary & seldom-used](12-supplementary/crew-overboard-prevention.md) | Prevention, rare events, tender, provisioning, handover |

## Master checklist index

### 00 — Boat profile
- [ ] [Boat profile](00-boat-profile/boat-profile.md) — *fill in with your boat's details*

### 01 — Pre-departure
- [x] [Passage planning](01-pre-departure/passage-planning.md)
- [x] [Boat & safety check](01-pre-departure/boat-safety-check.md)
- [x] [Crew briefing](01-pre-departure/crew-briefing.md)
- [x] [Fuel & water check](01-pre-departure/fuel-water-check.md)
- [x] [Weather & tide review](01-pre-departure/weather-tide-review.md)

### 02 — Sail handling
- [x] [Hoisting the mainsail](02-sail-handling/hoisting-mainsail.md)
- [x] [Reefing the mainsail](02-sail-handling/reefing-mainsail.md)
- [x] [Shaking out a reef](02-sail-handling/shaking-out-reef.md)
- [x] [Dropping / flaking the mainsail](02-sail-handling/dropping-flaking-mainsail.md)
- [x] [Headsail hoist / furl](02-sail-handling/headsail-furl.md)
- [x] [Headsail change](02-sail-handling/headsail-change.md)
- [x] [Spinnaker set / douse](02-sail-handling/spinnaker-set-douse.md)

### 03 — Maneuvers
- [x] [Tacking](03-maneuvers/tacking.md)
- [x] [Gybing](03-maneuvers/gybing.md)
- [x] [Heaving-to](03-maneuvers/heaving-to.md)
- [x] [Slowing, stopping & ferry gliding](03-maneuvers/slowing-stopping.md)
- [x] [Anchoring](03-maneuvers/anchoring.md)
- [x] [Weighing (raising) anchor](03-maneuvers/weighing-anchor.md)
- [x] [Picking up a mooring](03-maneuvers/picking-up-mooring.md)
- [x] [Towing & being towed](03-maneuvers/towing-and-being-towed.md)

### 04 — Docking & berthing
- [x] [Departure from berth](04-docking-berthing/departure-from-berth.md)
- [x] [Arrival / coming alongside](04-docking-berthing/coming-alongside.md)
- [x] [Spring-line maneuvers](04-docking-berthing/spring-line-maneuvers.md)
- [x] [Med mooring](04-docking-berthing/med-mooring.md)

### 05 — Emergencies
- [x] [Man overboard (MOB)](05-emergencies/man-overboard.md)
- [x] [Fire](05-emergencies/fire.md)
- [x] [Flooding / hull breach](05-emergencies/flooding.md)
- [x] [Dismasting](05-emergencies/dismasting.md)
- [x] [Steering failure](05-emergencies/steering-failure.md)
- [x] [Engine failure](05-emergencies/engine-failure.md)
- [x] [Medical emergency](05-emergencies/medical-emergency.md)
- [x] [Gas leak (LPG)](05-emergencies/gas-leak.md)
- [x] [Abandon ship](05-emergencies/abandon-ship.md)
- [x] [Grounding](05-emergencies/grounding.md)

### 06 — Heavy weather
- [x] [Heavy-weather preparation](06-heavy-weather/heavy-weather-preparation.md)
- [x] [Storm tactics](06-heavy-weather/storm-tactics.md)

### 07 — Navigation & watch
- [x] [Watch handover](07-navigation-watch/watch-handover.md)
- [x] [Night sailing prep](07-navigation-watch/night-sailing-prep.md)
- [x] [Landfall & arrival pilotage](07-navigation-watch/landfall-arrival-pilotage.md)
- [x] [Fog / restricted visibility](07-navigation-watch/fog-restricted-visibility.md)
- [x] [Log keeping](07-navigation-watch/log-keeping.md)

### 08 — Systems & engine
- [x] [Engine pre-start / start](08-systems-engine/engine-pre-start-start.md)
- [x] [Engine shutdown](08-systems-engine/engine-shutdown.md)
- [x] [Electrical / battery management](08-systems-engine/electrical-battery-management.md)
- [x] [Through-hull / seacock check](08-systems-engine/through-hull-seacock-check.md)
- [x] [Heads (toilet) operation & blockage](08-systems-engine/heads-operation-blockage.md)

### 09 — Maintenance
- [x] [Daily checks](09-maintenance/daily-checks.md)
- [x] [Weekly checks](09-maintenance/weekly-checks.md)
- [x] [Seasonal / lay-up & recommission](09-maintenance/seasonal-layup-recommission.md)

### 10 — Radio protocols
- [x] [Mayday (distress)](10-radio-protocols/mayday-distress.md)
- [x] [PAN-PAN (urgency)](10-radio-protocols/pan-pan-urgency.md)
- [x] [Sécurité (safety)](10-radio-protocols/securite-safety.md)
- [x] [Routine call & radio check](10-radio-protocols/routine-call-radio-check.md)
- [x] [DSC — digital selective calling](10-radio-protocols/dsc-digital-selective-calling.md)
- [x] [Phonetic alphabet & numbers](10-radio-protocols/phonetic-alphabet.md)

### 11 — COLREGs quick-reference
- [x] [Right of way & steering rules](11-colregs-quick-reference/right-of-way-steering-rules.md)
- [x] [Navigation lights](11-colregs-quick-reference/navigation-lights.md)
- [x] [Day shapes](11-colregs-quick-reference/day-shapes.md)
- [x] [Sound signals](11-colregs-quick-reference/sound-signals.md)

### 12 — Supplementary & seldom-used
- [x] [Sea-sickness management](12-supplementary/sea-sickness-management.md)
- [x] [Crew-overboard prevention](12-supplementary/crew-overboard-prevention.md)
- [x] [Collision-avoidance decision](12-supplementary/collision-avoidance-decision.md)
- [x] [Collision (aftermath)](12-supplementary/collision-aftermath.md)
- [x] [Lightning / electrical storm](12-supplementary/lightning-electrical-storm.md)
- [x] [Cold water & hypothermia](12-supplementary/cold-water-hypothermia.md)
- [x] [Gas system check](12-supplementary/gas-system-check.md)
- [x] [Bilge & pump test](12-supplementary/bilge-pump-test.md)
- [x] [Winch safety & riding turn](12-supplementary/winch-safety-riding-turn.md)
- [x] [Dinghy / tender operation](12-supplementary/dinghy-tender-operation.md)
- [x] [Provisioning & stowage](12-supplementary/provisioning-stowage.md)
- [x] [Guest / charter handover](12-supplementary/guest-charter-handover.md)

## Contributing convention

- One checklist per file, kebab-case filename.
- Use the shared template in [TEMPLATE.md](TEMPLATE.md).
- Keep items imperative and scannable. Bold the killer items.

## Get it on your phone

Open the site anywhere — it works offline once loaded, has instant search,
and light/dark mode:

**https://amir-daniel.github.io/sailing-protocols/**

Scan to open it on your phone:

![Scan to open the site on your phone](assets/qr/site-home.png)

> Maintaining this site (build, deploy, regenerating QR codes) is documented in
> [MAINTAINING.md](MAINTAINING.md).

